...
def analyze_data(path):
   ...
   if match:
       ...
       Wstr = str(W)  
   pattern1 = re.compile(r'([\w\d/]+)/results')
   ...
   plt.text(np.max(bin_centers)/2, counts[0]/1.4,  text)
   plt.text(np.max(bin_centers)/2, counts[0]/1.5,  Atext)
   plt.text(np.max(bin_centers)/2, counts[0]/1.6,  tautext)
   plt.text(np.max(bin_centers)/2, counts[0]/1.8, tautextapprox)
   plt.text(np.max(bin_centers)/2, counts[0]/2.0, tautextcalc)
